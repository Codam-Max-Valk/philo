...
int	init_params(t_params *s_data)
{
	...
	pthread_mutex_init(&s_data->start_lock, NULL);
	init_forks(s_data);
	return (0);
}
...
int	init_philosophers(int ac, char **av, t_params *s_data)
{
	if (ac == 6)
	{
		s_data->max_eat = ft_atol(av[--ac]);
		s_data->eat_limit = true;
		if (s_data->max_eat < 1)
			return (-1);
	}
	s_data->time_to_sleep = ft_atol(av[--ac]);
	s_data->time_to_eat = ft_atol(av[--ac]);
	s_data->time_to_die = ft_atol(av[--ac]);
